...
const { handleSiteEvents } = require("../lib/server/sms-funnel-orchestrator");
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!Array.isArray(events) || events.length === 0) {
      return sendJson(res, 400, { ok: false, error: "missing_events_array" });
    }

    const out = await handleSiteEvents({ slug, salonName, events });
