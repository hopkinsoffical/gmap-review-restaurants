#!/usr/bin/env python3
"""
Fetch salon/spa places for Maryland townships via Google Places API (New),
dedupe by place_id, ingest into public.salon_ai_leaderboard, then optionally
enrich Instagram for the MD rows.

Targets are read from data/md_townships_salon_targets.txt with one row per line:
...
Usage (repo root):
  python3 pipelines/fetch_ingest_md_townships_salons.py --dry-run
  python3 pipelines/fetch_ingest_md_townships_salons.py
  python3 pipelines/fetch_ingest_md_townships_salons.py --enrich-instagram
"""
...
DEFAULT_TARGETS_PATH = ROOT / "data" / "md_townships_salon_targets.txt"
...
def search_specs_for_md_town(town_label: str) -> list[tuple[str, str]]:
    t = (town_label or "").strip()
    if not t:
        raise ValueError("town_label is required")
    return [
        ("hair_care", f"hair salon {t} Maryland"),
        ("beauty_salon", f"beauty salon {t} Maryland"),
        ("nail_salon", f"nail salon {t} Maryland"),
        ("spa", f"day spa {t} Maryland"),
        ("barber_shop", f"barber shop {t} Maryland"),
    ]


def load_md_markets(path: Path) -> list[dict[str, Any]]:
    if not path.is_file():
...
def collect_unique_ids_with_fallbacks(
    ...
    for i, m in enumerate(markets, 1):
        ...
        rm = m.get("radius_m")
        specs = search_specs_for_md_town(label)
        print(f"[{i}/{len(markets)}] Searching Places ({label}, MD)…", flush=True)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Fetch MD township salons/spas and ingest to leaderboard.")
    parser.add_argument(
        "--targets-file",
        type=Path,
        default=DEFAULT_TARGETS_PATH,
        help="Pipe-delimited targets file (default: data/md_townships_salon_targets.txt)",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Only write data/md-townships-salons-fetched.json (no Supabase)",
    )
    parser.add_argument(
        "--max-targets",
        type=int,
    ...
    parser.add_argument(
        "--enrich-instagram",
        action="store_true",
        help="Run scripts/leaderboard_instagram_enrichment.py for state=MD after ingest",
    )
    parser.add_argument(
        "--instagram-sleep",
        type=float,
    ...
    markets = load_md_markets(targets_path)
    if args.max_targets and args.max_targets > 0:
        markets = markets[: int(args.max_targets)]
    print(f"Loaded {len(markets)} MD township target(s) from {targets_path}", flush=True)
    ...
    out_path = ROOT / "data" / "md-townships-salons-fetched.json"
