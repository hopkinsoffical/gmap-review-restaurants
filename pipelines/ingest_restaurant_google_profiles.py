#!/usr/bin/env python3
"""
Ingest AI leaderboard source JSON into public.info_gather_google_profiles (append-only history).
...
Usage (from repo root):
  python3 pipelines/ingest_info_gather_google_profiles.py [path/to/source.json]
  AI_LEADERBOARD_SOURCE_PATH=data/ai-leaderboard-source.json python3 pipelines/ingest_info_gather_google_profiles.py
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Ingest info_gather_google_profiles rows from JSON.")
    parser.add_argument(
        "source_path",
        nargs="?",
