...
const { getDefaultSalonMenuJsonSafe } = require("./default-menu");
...
async function resolveStoreMenuPack(bootstrap) {
  if (!bootstrap || !bootstrap.store) return null;

  const published = bootstrap.menuSnapshot;
  const publishedMenu = published && published.menu && typeof published.menu === "object" ? published.menu : null;
  if (publishedMenu) {
    return {
      menuJson: publishedMenu,
      menuSnapshotId: published.id,
      catalogStatus: "published",
    };
  }

  try {
    const servicePack = await listServiceItemsByStoreIdWithStatus(bootstrap.store.id);
    const items = Array.isArray(servicePack.items) ? servicePack.items : [];
    const activeNamed = items.filter(function (item) {
      return item && item.isActive !== false && String(item.name || "").trim();
    });
    if (activeNamed.length > 0) {
      return {
        menuJson: buildMenuJsonFromPortalServiceItems(items),
        menuSnapshotId: null,
        catalogStatus: "portal_services",
      };
    }
  } catch (err) {
    const code = err && err.code ? String(err.code) : "";
    if (code !== "SERVICE_ITEMS_POSTGREST_NOT_VISIBLE" && code !== "SERVICE_ITEMS_TABLE_MISSING") {
      throw err;
    }
  }

  const fallbackMenu = getDefaultSalonMenuJsonSafe();
  if (!fallbackMenu) return null;
  return {
    menuJson: fallbackMenu,
    menuSnapshotId: null,
    catalogStatus: "default_seed",
  };
}
