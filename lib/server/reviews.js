...
const FOCUS_RULES = {
  none: {
    zhRule: "这一条只写项目本身，比如颜色、细致度、成品效果、保持度，完全不要提staff、环境、氛围。",
    enRule:
  ...
  service: {
    ...
    enRule:
      "This review must clearly praise the staff, attentiveness, communication, care, or technique, but must not mention ambiance, atmosphere, decor, environment, or the salon space.",
  },
  environment: {
...
const SERVICE_PRAISE_OPTIONS = [
  ...
  { key: "detailed", zhLabel: "做得细致", enLabel: "very detailed", zhPhrase: "做得细致", enPhrase: "very detailed" },
  { key: "helpful", zhLabel: "沟通很顺", enLabel: "easy to communicate with", zhPhrase: "沟通很顺", enPhrase: "easy to communicate with" },
  { key: "gentle", zhLabel: "手法很温柔", enLabel: "gentle and careful", zhPhrase: "手法很温柔", enPhrase: "gentle and careful" },
];
...
const REVIEW_PATTERN_TYPES = {
  staff_led: {
    ...
    zhRule: "先点人，再点项目，再落一个可信细节，最后只放一个轻的回头或推荐信号。",
    enRule:
  ...
  repeat_customer: {
    ...
    zhRule: "先点出不是第一次来，再写这次的项目或稳定发挥，最后轻轻落一个继续会来的信号。",
    enRule:
  ...
  service_result: {
    ...
    zhRule: "先点项目，再写效果或成品，再补一个可信细节，最后只放一个轻收尾。",
    enRule:
      "Start with the service, then the result, add one believable proof point, and finish with one light closing signal.",
  },
};
...
const REVIEW_PROOF_TYPES = {
  skill: {
    zhLabel: "手法或细节证明",
    enLabel: "skill proof",
    zhRule: "用细致、认真、手法稳、沟通顺之类的真实细节证明不是空夸。",
  ...
  cleanliness: {
    ...
    enRule: "When mentioning the salon setting, prefer very clean, spotless, bright, or comfortable over marketing-style phrasing.",
  },
  warmth: {
    zhLabel: "热情舒适证明",
...
const KEYWORD_REALIZATION_MAP = {
  ...
  gentle: {
    ...
    en: ["gentle", "super gentle", "really gentle with my nails"],
  },
  polished: {
...
function buildSingleReviewMessages(store, menuContext, dishIds, lang, recentTexts, retryLevel, visitTierKey, servicePraise, cardPlan, extraAvoidTexts) {
  ...
  if (lang === "zh") {
    return {
      system: "你只写 1 条真实顾客口吻的 Google Maps 美甲店短评。像刚结束服务顺手发出，不像广告。只返回 JSON。",
      user:
        "店铺：" +
        store.nameZh +
        "\n输出语言：中文\nstyle_key=" +
  ...
  return {
    system: "Write exactly one believable first-person Google Maps salon review. Sound like a real client right after one appointment. Return JSON only.",
    user:
      "Salon: " +
...
function describeFocusDirective(focusKey, lang) {
  if (lang === "zh") {
    if (focusKey === "service") return "必须提 staff、照顾感、沟通或手法；不要提干净、环境、氛围、空间。";
    if (focusKey === "environment") return "必须提干净、舒服、氛围或空间；不要提 staff、老师、美甲师或具体人名。";
    return "只写项目效果、成品、颜色、形状、亮泽或保持度；不要提 staff，也不要提干净、欢迎感、放松氛围或空间。";
  }
  if (focusKey === "service") {
    return "Must mention the person, staff care, technique, or communication. Do not mention cleanliness, atmosphere, decor, the place, or the salon setting.";
  }
  if (focusKey === "environment") {
    return "Must mention cleanliness, comfort, atmosphere, or the salon setting. Do not mention staff, attentiveness, technique, or any named person.";
  }
  return "Result only. Mention the service outcome, finish, shape, color, shine, or wear. Do not mention staff, friendliness, patience, cleanliness, welcoming, comfort, atmosphere, or the salon setting.";
}
...
function buildReviewMessages(store, menuContext, dishIds, lang, recentTexts, retryLevel, focusAssignments, lengthAssignments, visitTierKey, servicePraise, keywordAssignments, patternAssignments, surfaceAssignments) {
  ...
  if (lang === "zh") {
    return {
      system:
        "你在帮真实顾客写 Google Maps 美甲店短评。用第一人称，像一个人刚结束一次服务后顺手发出的评论，不像广告，不像关键词清单。每条只围绕一次真实到店，最多提 1 到 2 个项目，只放 1 个可信 proof 点和 1 个收尾信号。只返回 JSON。",
      user:
        "店铺：" +
        store.nameZh +
        "\n输出语言：中文\n\n这次做的项目：\n" +
        ...
        "4. 每条最多提 1 到 2 个项目，不要把服务项目一股脑塞进去。\n" +
  ...
  return {
    system:
      "You write believable Google Maps salon reviews in first-person customer voice. Sound like one real person describing one real visit, not a brand or an SEO checklist. Each review should mention at most 1 or 2 services, include one believable proof point, and end with one light closing signal. Return JSON only.",
    user:
      "Salon: " +
      getRestaurantLabel(store) +
      "\nOutput language: English\n\nServices from this visit:\n" +
...
function validateReviewContent(review, lang, keywordAssignment, servicePraise, options) {
  ...
  const environmentPattern =
    lang === "zh"
      ...
      : /\bambiance\b|\batmosphere\b|\bdecor\b|\benvironment\b|\bspace\b|\bsetting\b|\broom\b|\bvibe(?:s)?\b|\bcozy\b|\bspotless\b|\bvery clean\b(?! (result|finish|set|look))|\bclean and welcoming\b|\bbright and clean\b|\bclean and professional\b|\bwell kept\b|\bclean (salon|space|room|place)\b|\b(salon|space|room|place) (was |felt )?clean\b|\b(relaxing|calm|comfortable) (space|room|setting|vibe|atmosphere|salon)\b|\brelaxing atmosphere\b/i;
  ...
  if (review.focus === "none" && (hasService || hasEnvironment)) {
    throw createAppError("INVALID_REVIEW_FOCUS", "Results-only review mentioned staff or salon environment", 502);
  }
...
function buildEnglishEnvironmentClause(phrase) {
  const value = String(phrase || "").trim();
  if (!value) return "The salon felt very clean and relaxing";
  if (/atmosphere/i.test(value)) return "The salon had a " + value;
  return "The salon felt " + value;
}

function buildEnglishResultClause(phrase) {
  const value = String(phrase || "").trim();
  if (!value) return "The result came out polished";
  if (/^looked /i.test(value) || /^came out /i.test(value)) return "The result " + value;
  ...
  if (/finish$|shine$/i.test(value)) return "The result had a " + value;
  if (/precise/i.test(value)) return "The shape was " + value.replace(/^really\s+/i, "");
  return "The result felt " + value;
}
...
function buildFallbackCandidateTexts(cardPlan, servicePraise, lang) {
  ...
  if (lang === "zh") {
    ...
    return keywordPhrases.flatMap(function (phrase, index) {
      if (styleKey === "review_a") {
        return [
          phrase + "，线条也顺，" + closings[index % closings.length],
          phrase + "，这次做完看着就很顺眼。",
        ];
      }
      if (styleKey === "review_b") {
        return [
          "这次做出来特别利落，" + phrase + "，颜色和形状都很在线，" + closings[index % closings.length],
          "成品这次很加分，" + phrase + "，细节看着也很干净。",
        ];
      }
      return [
        "最喜欢的是成品出来很顺，" + phrase + "，看一眼就觉得值，" + closings[index % closings.length],
        phrase + "，颜色和形状都很顺眼，也挺耐看。",
      ];
    });
  }
...
function buildGuaranteedFallbackText(cardPlan, servicePraise, lang) {
  ...
  if (lang === "zh") {
    if (cardPlan.focusKey === "service") {
      if (cardPlan.styleKey === "review_a") return buildZhServiceClause("", staffLabel, servicePraise) + "，这次也很满意。";
      if (cardPlan.styleKey === "review_b") return "这次想夸夸" + staffLabel + "，沟通很顺，细节也做得到位，下次还会来做。";
      return "最记得的是" + staffLabel + "做事很稳，照顾得也到位，会想再来。";
    }
    if (cardPlan.focusKey === "environment") {
      if (cardPlan.styleKey === "review_a") return "店里很干净，坐着也舒服，整体很满意。";
      if (cardPlan.styleKey === "review_b") return "环境这次特别加分，空间很舒服，待着不会有压力，下次还会来做。";
      return "让我记住的是店里很放松，环境也收拾得很利落，会推荐给喜欢舒服一点的人。";
    }
    if (cardPlan.styleKey === "review_a") return "成品看着很自然，这次也很满意。";
    if (cardPlan.styleKey === "review_b") return "这次做出来很利落，颜色和形状都在线，下次还会来做。";
    return "最喜欢的是成品出来很顺，看着就觉得值，比预期还好。";
  }

  if (cardPlan.focusKey === "service") {
    if (cardPlan.styleKey === "review_a") return buildEnglishServiceClause("", staffLabel) + ". Really happy with how it came out.";
    if (cardPlan.styleKey === "review_b") return buildEnglishServiceClause("easy to communicate with", staffLabel) + ". The little details looked great. Would come back for this.";
    return "The part I remember most is that " + buildEnglishServiceClause("very detailed", staffLabel).replace(/\.$/, "").toLowerCase() + ". It made the whole visit stand out.";
  }
  if (cardPlan.focusKey === "environment") {
    if (cardPlan.styleKey === "review_a") return "The salon felt very clean and comfortable. Really happy with how it came out.";
    if (cardPlan.styleKey === "review_b") return "The space felt clean, calm, and easy to relax in. Would come back for this.";
    return "What stood out most is that the salon felt relaxing and well kept. Better than I expected.";
  }
  if (cardPlan.styleKey === "review_a") return "The result looked natural and polished. Really happy with how it came out.";
  if (cardPlan.styleKey === "review_b") return "The shape and finish both looked really clean. Would come back for this.";
  return "What I noticed most is that the result came out smooth and even. Better than I expected.";
}
