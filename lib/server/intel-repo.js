...
const { listRestrLeaderboard } = require("./restr-leaderboard-repo");

const INTEL_READ_SOURCE = "restr_ai_leaderboard_latest";
const INTEL_LIST_SEARCH_MIN = 2;
const INTEL_LIST_DEFAULT_LIMIT = 20;
...
async function listIntelSalons(options) {
  ...
  const result = await listRestrLeaderboard({
    search: search.length >= INTEL_LIST_SEARCH_MIN ? search : undefined,
    limit: opts.limit,
    offset: opts.offset,
    state: opts.state,
    county: opts.county,
